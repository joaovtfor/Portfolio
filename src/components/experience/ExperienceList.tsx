      ...
      {experiences.map((exp, i) => (
        ...
        >
          ...
          <div className={`hidden md:block absolute left-0 top-0 bottom-0 w-[1px] transition-colors duration-500 ${i === experiences.length - 1 ? 'bg-gradient-to-b from-white/5 to-transparent group-hover:from-white/10' : 'bg-white/5 group-hover:bg-white/10'}`}>
            <div className="absolute top-12 -left-[5px] w-3 h-3 rounded-full border border-white/20 bg-neutral-900 group-hover:bg-[var(--foreground)] group-hover:border-[var(--foreground)] group-hover:shadow-[0_0_12px_var(--foreground)] transition-all duration-700" />
          </div>
